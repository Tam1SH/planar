...
/// 2. Implement `crate::traits::Convert<S>` for `ServerDef`.
...
///
/// **Requirement:**
/// You must implement `crate::parser::convert::Convert<S>` for the struct/enum.
///
/// **Generated Method:**
...
pub fn planar_node(_: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let wrapper_struct = quote! {
        #[derive(Clone)]
        #vis struct #original_ident(pub crate::parser::spanned::Spanned<#data_ident>);
    ...
    let context_struct = quote! {
        ...
        #vis struct #context_ident {
            pub ctx: crate::parser::ctx::ParseContext,
        }
    };
    ...
    let common_impls = quote! {
        ...
        impl #original_ident {
            pub fn span(&self) -> &crate::parser::ctx::ParseContext {
                &self.0.ctx
            }
        ...
        impl From<crate::parser::spanned::Spanned<#data_ident>> for #original_ident {
            fn from(spanned: crate::parser::spanned::Spanned<#data_ident>) -> Self {
                Self(spanned)
            }
        }
    };
    ...
    let kdl_impls = quote! {
        impl<S> crate::parser::parsable::KdlParsable<S> for #original_ident {
            fn parse_node(
                ctx: &crate::parser::ctx::ParseContext,
                state: &S
            ) -> Result<Self, crate::error::ConfigError> {
                let data = <#data_ident as crate::parser::parsable::KdlParsable<S>>::parse_node(ctx, state)?;
                Ok(#original_ident(crate::parser::spanned::Spanned::new(data, ctx.clone())))
            }
        }

        impl crate::parser::node_schema::NodeSchema for #original_ident {
            fn applicable_node_names() -> &'static [&'static str] {
                #data_ident::applicable_node_names()
            }

            fn match_score(ctx: &crate::parser::ctx::ParseContext) -> (isize, Option<String>) {
                #data_ident::match_score(ctx)
            }
        }

        impl crate::schema::definitions::GetSchema for #original_ident {
            fn schemas(ctx: &mut crate::schema::schema_context::SchemaContext) -> Vec<crate::schema::definitions::NodeSchema> {
    ...
    let proxy_method = quote! {
        ...
        pub fn parse_as<S, T>(
            ctx: &crate::parser::ctx::ParseContext,
            state: &S
        ) -> Result<T, crate::error::ConfigError>
        where
            Self: crate::parser::parsable::KdlParsable<S> + crate::parser::convert::Convert<S, Output = T>,
        {
            let schema = <Self as crate::parser::parsable::KdlParsable<S>>::parse_node(ctx, state)?;

            match crate::parser::convert::Convert::convert(schema, state) {
                Ok(val) => Ok(val),

                Err(e) => {
                    let report: miette::Report = e.into();

                    let parse_err = crate::parser::utils::macros_helpers::to_parse_error(
                        report,
                        ctx.current_span(),
                        ctx.source().clone()
                    );
                    Err(crate::error::ConfigError::from_list(vec![parse_err]))
                }
            }
        }
        }
