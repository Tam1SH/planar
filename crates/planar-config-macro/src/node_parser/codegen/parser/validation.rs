...
impl<'a> ValidationGenerator<'a> {
    ...
    pub fn gen_global_rules(
        ...
        match block {
            BlockSpec::Empty => rules.push(quote!(crate::parser::ensures::Rule::NoChildren)),
            _ if !allow_empty_block => {
                rules.push(quote!(crate::parser::ensures::Rule::ReqChildren))
            }
            _ => {}
        }
        ...
        if !is_root {
            if has_all_args {
                ...
                if arg_count == 0 {
                    rules.push(quote!(crate::parser::ensures::Rule::NoPositionalArgs));
                } else {
            ...
            if !has_all_props {
                let prop_schema = props.iter().map(|p| {
                    ...
                    quote!((#k, crate::parser::utils::PrimitiveType::#pt))
                });
                rules.push(
                    quote!(crate::parser::ensures::Rule::OnlyKeysTyped(&[ #(#prop_schema),* ])),
                );
            }
        }
