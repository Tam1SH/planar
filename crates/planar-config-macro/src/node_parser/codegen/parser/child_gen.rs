...
impl<'a> ChildGenerator<'a> {
    ...
    fn gen_dynamic_block(
        ...
        let parse_call = if let Some(func) = &opts.parse_with {
            ...
            quote!(<#inner_type as crate::parser::parsable::KdlParsable<S>>::parse_node(&child_ctx, state))
        };
    ...
    fn gen_strict_block(&self, children: &[ChildSpec], ignore_unknown: bool) -> TokenStream {
        ...
        decls.push(quote! {
            let mut __children_map: std::collections::HashMap<String, Vec<crate::parser::ctx::ParseContext>> =
        ...
        for child in children {
            ...
            let names_expr = if let Some(n) = &child.name {
                ...
                quote! { <#inner as crate::parser::node_schema::NodeSchema>::applicable_node_names() }
            } else {
                match child.mode {
                    ChildMode::Field => {
                    ...
                    ChildMode::Node => {
                        ...
                        quote! {
                            {
                                let schema_names = <#inner as crate::parser::node_schema::NodeSchema>::applicable_node_names();
            ...
            let parse_call = if let Some(func) = &opts.parse_with {
                ...
                match child.mode {
                    ...
                    crate::node_parser::model::ChildMode::Node => {
                        quote!(<#inner as crate::parser::parsable::KdlParsable<S>>::parse_node(&child_ctx, state))
                    }
                }
            };
