...
impl<'a> EnumGenerator<'a> {
    ...
    fn gen_variant_body(&self, v: &VariantSpec) -> TokenStream {
        ...
        match &v.fields {
            ...
            VariantFields::Newtype(ty) => quote! {
                let data = <#ty as crate::parser::parsable::KdlParsable<S>>::parse_node(ctx, state)?;
                Ok(Self::#ident(data))
            },
    ...
    pub fn gen_scoring_helper(&self) -> TokenStream {
        ...
        quote! {
            impl #struct_name {
                #[doc(hidden)]
                pub fn __kdl_score_variants(ctx: &crate::parser::ctx::ParseContext) -> [(isize, Option<String>); #count] {
                    [ #(#calcs),* ]
                }
            }
        }
