...
impl<'a> SchemaGenerator<'a> {
    ...
    pub fn generate(&self) -> TokenStream {
        ...
        quote! {
            impl crate::schema::definitions::GetSchema for #struct_name {
        ...
        quote! {
            {
                ...
                if !ctx.enter(id) {
                    return vec![crate::schema::definitions::NodeSchema {
                        ...
                        args: vec![],
                        props: vec![],
                        children: crate::schema::definitions::ChildrenSchema::Recursive(id.to_string()),
                    }];
                }

                let res = vec![
                    crate::schema::definitions::NodeSchema {
                        matcher: #matcher,
        ...
        let schemas = variants.iter().map(|v| {
            ...
            match &v.fields {
                VariantFields::Unit => {
                    ...
                    quote! {
                        vec![crate::schema::definitions::NodeSchema {
                            matcher: crate::schema::definitions::NodeNameMatcher::Keyword(#name.to_string()),
                            description: std::borrow::Cow::Borrowed(#docs),
                            examples: vec![],
                            args: vec![],
                            props: vec![],
                            children: crate::schema::definitions::ChildrenSchema::None,
                        }]
                    }
                }
                VariantFields::Newtype(inner_ty) => {
                    ...
                    if let Some(name) = override_name {
                        ...
                        quote! {
                            <#inner_ty as crate::schema::definitions::GetSchema>::schemas(ctx)
                                .into_iter()
                                .map(|mut s| {
                                    s.matcher = crate::schema::definitions::NodeNameMatcher::Keyword(#name.to_string());
        ...
        quote! {
            {
                let mut all = Vec::new();
                #(
                    let variant_schemas: Vec<crate::schema::definitions::NodeSchema> = #schemas;
    ...
    fn gen_matcher(
        ...
        if let Some(field) = node_name_field {
            ...
            quote! {
                crate::schema::definitions::NodeNameMatcher::Variable {
                    label: #label.to_string()
                }
            }
        } else if let Some(name) = kdl_name {
            quote! {
                crate::schema::definitions::NodeNameMatcher::Keyword(#name.to_string())
            }
        } else {
            // Fallback, though usually logic shouldn't reach here for unnamed nodes
            quote! {
                crate::schema::definitions::NodeNameMatcher::Keyword("<unknown>".to_string())
            }
        }
    }
    ...
    fn gen_args(&self, args: &[ArgSpec]) -> TokenStream {
        let items = args.iter().map(|arg| {
            ...
            quote! {
                crate::schema::definitions::ArgSchema {
                    name: #name.to_string(),
                    description: std::borrow::Cow::Borrowed(#docs),
                    kind: #kind,
                    required: #req,
    ...
    fn gen_props(&self, props: &[PropSpec]) -> TokenStream {
        let items = props.iter().map(|prop| {
            ...
            quote! {
                crate::schema::definitions::PropSchema {
                    name: #name.to_string(),
                    description: std::borrow::Cow::Borrowed(#docs),
                    kind: #kind,
                    required: #req,
    ...
    fn gen_children_block(&self, block: &BlockSpec) -> TokenStream {
        match block {
            BlockSpec::Empty => quote!(crate::schema::definitions::ChildrenSchema::None),
            BlockSpec::Strict(children) => {
                ...
                quote! {
                    crate::schema::definitions::ChildrenSchema::Fixed({
            ...
            BlockSpec::Dynamic { inner_type, .. } => {
                ...
                quote! {
                     crate::schema::definitions::ChildrenSchema::Fixed(
                        <#inner_type as crate::schema::definitions::GetSchema>::schemas(ctx)
                     )
                }
            }
        }
    ...
    fn gen_child_schema_call(&self, child: &ChildSpec) -> TokenStream {
        ...
        if child.mode == crate::node_parser::model::ChildMode::Field {
            ...
            quote! {
                vec![
                    crate::schema::definitions::NodeSchema {
                        matcher: crate::schema::definitions::NodeNameMatcher::Keyword(#node_name.to_string()),
                        description: std::borrow::Cow::Borrowed(#docs),
                        examples: vec![],
                        args: vec![
                            crate::schema::definitions::ArgSchema {
            ...
            if let Some(name) = &child.name {
                quote! {
                    <#target_type as crate::schema::definitions::GetSchema>::schemas(ctx)
                        .into_iter()
                        .map(|mut s| {
                            s.matcher = crate::schema::definitions::NodeNameMatcher::Keyword(#name.to_string());
                            s
    ...
    fn gen_value_kind(
        ...
        if let Some(schema_name) = &opts.schema_name {
            return quote! {
                crate::schema::definitions::ValueKind::Catalog {
        ...
        quote! {
            <#ty as crate::schema::value_info::KdlValueInfo>::value_kind()
        }
    }
